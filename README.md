...
一个可以直接安装到 Windows 版 Codex 桌面端的火影忍者风格 CodeDrobe 主题。它包含主题源码、自动打包、Windows Store/MSIX 启动兼容、持续 watcher、验证截图和一键恢复脚本。
...
5. 保持命令窗口开启，看到 `Installed and verified` 后即安装完成。
...
双击 **`Restore Codex Default.cmd`**。脚本会停止本项目拥有的 watcher，并调用 CodeDrobe 的 `restore` 恢复注入前的 Codex 外观设置。如果窗口颜色仍被系统缓存，再正常重启一次 Codex。
...
本项目为独立同人项目，与 OpenAI、CodeDrobe 或 Naruto 权利方无隶属或背书关系。
